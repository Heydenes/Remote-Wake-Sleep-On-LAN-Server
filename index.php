...
<html lang="en" >
  <head>
    ...
    <meta name="description" content="A wake on lan Web Tool">
    <meta name="author" content="Dinesh Budania">
...
<input type="hidden" name="logout" value="1">  
  		<input type="submit" value="Logout"/></span>
</form></div>
    	<form class="form-signin" method="post">
			...
			<?php
				...
				$selectedComputer = $_GET['computer'];
			 	...
			 	echo "Lookup Wake-On-LAN System: </h3>";
				if ($approved_wake) {
					echo "Waking Up!";
            ...
            <?php
				...
				if (!isset($_POST['submitbutton']) || (isset($_POST['submitbutton']) && !$approved_wake && !$approved_sleep))
				{
					...
					{
						$asleep = true;
						echo "<h5>" . $COMPUTER_NAME[$selectedComputer] . " is presently <span style='color:RED;'>shutdown</span>.</h5>";
					}
					else
					{
						$asleep = false;
						echo "<h5>" . $COMPUTER_NAME[$selectedComputer] . " is presently <span style='color:GREEN;'>ON</span>. No Action Needed!</h5>";
					}
				}
                ...
                if ($approved_wake)
                {
					...
					echo "<p>Command Sent. Waiting for " . $COMPUTER_NAME[$selectedComputer] . " to turn on...</p><p>";
					$count = 1;
					$down = true;
					while ($count <= $MAX_PINGS && $down == true)
					{
						...
						if ($pinginfo != "")
						{
							...
							echo "<p><a href='?computer=" . $selectedComputer . "'>Return to the Wake Control Home</a></p>";
							$show_form = false;
						}
						else
						{
				...
				elseif ($approved_sleep)
				{
					...
					else
					{
						...
						if ($down == false)
						{
							echo "<p style='color:#CC0000;'><b>FAILED!</b> " . $COMPUTER_NAME[$selectedComputer] . " doesn't seem to be falling asleep... Try Manually?</p><p>(Or <a href='?computer=" . $selectedComputer . "'>Return to the Wake Control Home</a>.)</p>";
						}
					}
					curl_close($ch);
				}
            ...
            ?>
                    ...
                    <?php if ( (isset($_POST['submitbutton']) && $_POST['submitbutton'] == "Wake Up!") || (!isset($_POST['submitbutton']) && $asleep) ) {?>
        				<input class="btn btn-large btn-primary" type="submit" name="submitbutton" value="TURN ON"/>
						<input type="hidden" name="submitbutton" value="TURN ON"/>  <!-- handle if IE used and enter button pressed instead of wake up button -->
                    <?php } else { ?>
		                <input class="btn btn-large btn-primary" type="submit" name="submitbutton" value="TURN OFF"/>
						<input type="hidden" name="submitbutton" value="TURN OFF" />  <!-- handle if IE used and enter button pressed instead of sleep button -->
                    <?php } 
